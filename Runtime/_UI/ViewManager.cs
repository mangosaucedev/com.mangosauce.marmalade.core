...
namespace Marmalade.UI
{
    public class ViewManager : GameSystem<ViewManager>
    {
        ...
        {
            Events.Listen<ViewOpenedEvent>(this, OnViewOpened);
            Events.Listen<ViewClosedEvent>(this, OnViewClosed);
        }

        private void OnDisable()
        {
            Events.StopListening<ViewOpenedEvent>(this);
            Events.StopListening<ViewClosedEvent>(this);
        }
        ...
        public static void Open(string viewName, bool locksInput = true, bool isInteractive = true)
        {
            ...
            GameObject prefab = Assets.Get<Asset<GameObject>>(viewName).Contents;
            Transform parent = Folders.Canvas;
            view.gameObject = Instantiate(prefab, parent);
            view.gameObject.name = view.gameObject.name.Replace("(Clone)", "");
            activeViews.Add(view);
            Events.Fire(new ViewOpenedEvent(view));
        }
        ...
        public static void Close(string viewName)
        {
            if (!TryFind(viewName, out ActiveView view))
                return;
            Events.Fire(new BeforeViewClosedEvent(view));
            activeViews.Remove(view);
            Destroy(view.gameObject);
            Events.Fire(new ViewClosedEvent(view));
        }
